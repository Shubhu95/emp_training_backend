...
class Crud_model extends CI_Model {
        ...
        try {
              ...
              /*$result=$s3->putObject(
                array(
                    ...
                    //'ContentType' => $_FILES['upload_video_file']['type'],
                    //'ACL'        => 'public-read'
                  )
              );*/
              ...
              $result=$s3->putObject(array(
                  ...
                  //'ContentDisposition' => 'inline; filename='.$keyName.'', //<-- and this !
  ...
  public function add_lesson() {
    ...
    if($lesson_type == 'video') {
      ...
      elseif ($lesson_provider == 'video_upload') {
        ...
        //$data['video_url'] = html_escape($this->input->post('video_upload_file'));
  ...
  public function get_category_id_new($slug = array()) {
    /*$category_details = $this->db->get_where('category', array('slug' => $slug))->row_array();*/
    $this->db->select('id');
    ...
    $category_details=array_column($category_details, 'id');
    return $category_details;
  }


  public function get_category_details_by_id_new($id) {
    //return $this->db->get_where('category', array('id' => $id));
  ...
  function filter_course_new($selected_category_id = array(), $selected_price = array(), $selected_level = array(), $selected_language = array(), $selected_rating = array()){
      
      
      
      $course_ids = array();
      //if ($selected_category_id != "all") {
      if(count($selected_category_id)>0){
      if (!in_array('all', $selected_category_id)) {
        $category_details = $this->get_category_details_by_id_new($selected_category_id);

        //echo "<pre>";
        //print_r($category_details);exit;
        
        $categories_ids_array=[];
        $sub_categories_ids_array=[];
        

        for($i=0;$i<count($category_details);$i++)
        {
            if($category_details[$i]['parent'] > 0)
            {
              array_push($sub_categories_ids_array, $category_details[$i]['id']);
            }

            else
            {
              array_push($categories_ids_array, $category_details[$i]['id']);
            }

            //array_push($categories_ids_array, $category_details[$i]['id']);
        }

        //$this->db->where(' category_id IN('.implode(',',$selected_category_id).') OR sub_category_id IN('.implode(',',$selected_category_id).') ');
        
        
        //$this->db->select('*');
        //$this->db->from('course');

        /*if(count($categories_ids_array)>0)
        {
          $this->db->where(' sub_category_id IN('.implode(',',$categories_ids_array).') OR category_id IN('.implode(',',$categories_ids_array).') ');
        }*/

        if(count($sub_categories_ids_array)>0)
        {
           $this->db->where_in('sub_category_id', $sub_categories_ids_array);  
        }

        if(count($categories_ids_array)>0)
        {
           $this->db->where_in('category_id', $categories_ids_array);  
        }
        
        //$this->db->or_where_in('sub_category_id', $sub_categories_ids_array);
      ...
      //if ($selected_price != "all") {
     if(count($selected_price)>0){
      if (!in_array('all', $selected_price) && count($selected_price)>0){
        //if ($selected_price == "paid") {
        if (in_array('paid', $selected_price) && !in_array('free', $selected_price)){
        //}elseif ($selected_price == "free") {
          $this->db->where('is_free_course IS NULL ');
        }

        if (in_array('free', $selected_price) && !in_array('paid', $selected_price)){
          $this->db->where('is_free_course IN(1) ');
        }
      ...
      //if ($selected_level != "all") {
      if(count($selected_level)>0){
      if (!in_array('all', $selected_level) && count($selected_level)>0){
        $this->db->where_in('level', $selected_level);
      }
      }

      //if ($selected_language != "all") {
      if(count($selected_language)>0){
      if (!in_array('all', $selected_language) && count($selected_language)>0){
        $this->db->where_in('language', $selected_language);
      }
      }
      ...
      foreach ($courses as $course) {
        if (!in_array('all', $selected_rating) && count($selected_rating)>0) {
          ...
          if ($number_of_ratings > 0) {
            $average_ceil_rating = ceil($total_rating / $number_of_ratings);
            //echo $average_ceil_rating;exit();
            if (in_array($average_ceil_rating, $selected_rating)) {
              array_push($course_ids, $course['id']);
            }
          }
        }else {
      ...
      if (count($course_ids) > 0) {
        $this->db->where_in('id', $course_ids);
        return $this->db->get('course')->result_array();
        //$this->db->get('course')->result_array();
        //echo $this->db->last_query();exit();
      }else {
        return array();
      }
    ...
    function filter_course_new_limit($selected_category_id = array(), $selected_price = array(), $selected_level = array(), $selected_language = array(), $selected_rating = array(), $limit, $offset){
     ...
     if(count($selected_price)>0){
      ...
      if (count($course_ids) > 0) {
        ...
        $query=$this->db->get();
        $courses_res=$query->result_array();

        //echo $this->db->last_query();exit();

        if($query->num_rows()>0)
        {
           //$courses_data=array_slice($courses_res, $offset, $limit);
          //echo "<pre>";
          //print_r($courses_res);exit;
    ...
    public function get_courses_by_course_adder($category_ids, $this_course_id, $adder_id)
    {
        $this->db->select('course.*, users.first_name, users.last_name');
        $this->db->from('course');
        $this->db->join('users', 'users.id=course.user_id', 'left');
        $this->db->where_in('course.category_id', $category_ids);
        $this->db->where_in('course.sub_category_id', $category_ids);
        $this->db->where('course.user_id', $adder_id);
        $this->db->where('course.id !=', $this_course_id);
        ...
        {
            $thumbnail_url= $this->crud_model->get_course_thumbnail_url($courses_res[$i]['id']);

            $courses_res[$i]['thumbnail_url']=$thumbnail_url;
            $total_rating =  $this->get_ratings('course', $courses_res[$i]['id'], true)->row()->rating;

            $number_of_ratings = $this->get_ratings('course', $courses_res[$i]['id'])->num_rows();

            if($number_of_ratings > 0 && !empty($total_rating))
            {
            ...
            $courses_res[$i]['average_rating']=$average_rating;
            ...
            $courses_res[$i]['total_rating']=$total_rating;


            if($courses_res[$i]['is_free_course'] == 1)
            {
               $courses_res[$i]['current_price']="Free";
               $courses_res[$i]['original_price']="";
            }

            else if($courses_res[$i]['discount_flag'] == 1)
            {
               $courses_res[$i]['current_price']=$courses_res[$i]['discounted_price'];
               $courses_res[$i]['original_price']=$courses_res[$i]['price'];

               if(!empty($courses_res[$i]['current_price']))
               {
                  $courses_res[$i]['current_price']= number_format((float)$courses_res[$i]['current_price'], 2, '.', ',');
               }

               if(!empty($courses_res[$i]['original_price']))
               {
                  $courses_res[$i]['original_price']= number_format((float)$courses_res[$i]['original_price'], 2, '.', ',');
               }
            }

            else
            {
               $courses_res[$i]['current_price']=$courses_res[$i]['price'];
               $courses_res[$i]['original_price']="";
    ...
    {
      $this->db->select("lesson.title as quiz_title, lesson.course_id, lesson.lesson_type, course.title as course_title");
      $this->db->from("lesson");
      $this->db->join("course", 'course.id=lesson.course_id');
      $this->db->where('lesson.id', $quiz_id);
      $query=$this->db->get();
      $result=$query->row();
      return $result;
    }
