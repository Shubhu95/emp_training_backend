...
class Home extends CI_Controller {
    ...
    public function courses() {
        ...
        if (isset($_GET['category']) && !empty($_GET['category'] && $_GET['category'] != "all") && count(json_decode($_GET['category']) != 0) ) {

           $category_array=json_decode($_GET['category']);
           $selected_category_id = $this->crud_model->get_category_id_new($category_array);
        }

        // Get the selected price
        if (isset($_GET['price']) && !empty($_GET['price']) && count(json_decode($_GET['price']) != 0) ) {
            $price_array=json_decode($_GET['price']);
            $selected_price = $price_array;
        }

        // Get the selected level
        if (isset($_GET['level']) && !empty($_GET['level']) && count(json_decode($_GET['level']) != 0) ) {
            $level_array=json_decode($_GET['level']);
            $selected_level = $level_array;
        }

        // Get the selected language
        if (isset($_GET['language']) && !empty($_GET['language']) && count(json_decode($_GET['language']) != 0) ) {
            $language_array=json_decode($_GET['language']);
            $selected_language = $language_array;
        }

        // Get the selected rating
        if (isset($_GET['rating']) && !empty($_GET['rating']) && count(json_decode($_GET['rating']) != 0)) {
            $rating_array=json_decode($_GET['rating']);
            $selected_rating = $rating_array;
        }
        ...
        {
        	if(count(json_decode($_GET['category'])) > 0)
        	{
	            if((!in_array('all', $selected_category_id)) && (in_array('all', json_decode($_GET['category']) )))
	        	{
	               array_push($selected_category_id, 'all');
	        	}
        	}

        	else if(count(json_decode($_GET['category'])) == 0)
        	{
        ...
        if(!empty($_GET['price']))
        {
        	if(count(json_decode($_GET['price'])) > 0)
        	{
	            if((!in_array('all', $selected_price)) && (in_array('all', json_decode($_GET['price']) )))
	        	{
	               array_push($selected_price, 'all');
	        	}
        	}

        	else if(count(json_decode($_GET['price'])) == 0)
        	{
        ...
        if(!empty($_GET['level']))
        {
        	if(count(json_decode($_GET['level'])) > 0)
        	{
	           if((!in_array('all', $selected_level)) && (in_array('all', json_decode($_GET['level']) )))
	        	{
	               array_push($selected_level, 'all');
	        	}  
        	}

        	else if(count(json_decode($_GET['level'])) == 0)
        	{
        ...
        {
        	if(count(json_decode($_GET['language'])) > 0)
        	{
	            if((!in_array('all', $selected_language)) && (in_array('all', json_decode($_GET['language']) )))
	        	{
	               array_push($selected_language, 'all');
	        	}
        	}

        	else if(count(json_decode($_GET['language'])) == 0)
        	{
        ...
        {
        	if(count(json_decode($_GET['rating'])) > 0)
        	{
	            if((!in_array('all', $selected_rating)) && (in_array('all', json_decode($_GET['rating']) )))
	        	{
	               array_push($selected_rating, 'all');
	        	}
        	}

        	else if(count(json_decode($_GET['rating'])) == 0)
        	{
        ...
        //echo "<pre>";
        //print_r(json_decode($_GET['rating']));exit;


        if (in_array('all', $selected_category_id) && in_array('all', $selected_price) && in_array('all', $selected_level) && in_array('all', $selected_language) && in_array('all', $selected_rating)) {
            ...
            if(!empty($_GET['category']))
            {
               $url_params .='?category='.urlencode($_GET['category']);
            }

            else
            {
               $url_params .='?category=%5B%5D';
            }


            if(!empty($_GET['price']))
            {
               $url_params .='&price='.urlencode($_GET['price']);
            }

            else
            {
                $url_params .='&price=%5B%5D';
            }


            if(!empty($_GET['level']))
            {
               $url_params .='&level='.urlencode($_GET['level']);
            }

            else
            {
                $url_params .='&level=%5B%5D';
            }


            if(!empty($_GET['language']))
            {
               $url_params .='&language='.urlencode($_GET['language']);
            }

            else
            {
                $url_params .='&language=%5B%5D';
            }


            if(!empty($_GET['rating']))
            {
               $url_params .='&rating='.urlencode($_GET['rating']);
            }

            else
            {
                $url_params .='&rating=%5B%5D';
            }
            ...
            $config['base_url']  = site_url('home/courses'.$url_params);
            $this->pagination->initialize($config);
            $this->db->where('status', 'active');
            $page_data['courses'] = $this->db->get('course', $config['per_page'], $page)->result_array();
        }

        else if (count(($selected_category_id)) == 0 && count(($selected_price)) == 0 && count(($selected_level)) == 0 && count(($selected_language)) == 0 && count(($selected_rating)) == 0) {
            $this->db->where('status', 'active');
            $total_rows = $this->db->get('course')->num_rows();
          ...
          //echo "<pre>";
          //print_r($selected_rating);exit;  
            $total_courses = $this->crud_model->filter_course_new($selected_category_id, $selected_price, $selected_level, $selected_language, $selected_rating);

            if($total_courses)
            {
               $total_rows=count($total_courses);
            }
            ...
            $courses = $this->crud_model->filter_course_new_limit($selected_category_id, $selected_price, $selected_level, $selected_language, $selected_rating, $config['per_page'], $page);
            //echo "<pre>";
        ...
        $page_data['limit']  = $config['per_page'];
        $page_data['offset']  = $page;
    ...
    public function course($slug = "", $course_id = "") {
        ...
        $this_course_id= $course_details['id'];
        $adder_id=$course_details['user_id'];
        $category_ids=[];
        if(!empty($course_details['category_id']))
        {
           array_push($category_ids, $course_details['category_id']);
        }

        if(!empty($course_details['sub_category_id']))
        {
           array_push($category_ids, $course_details['sub_category_id']);
        }

        //echo "<pre>";
        //print_r($category_ids);exit();

        $adder_other_course_details = $this->crud_model->get_courses_by_course_adder($category_ids, $this_course_id, $adder_id);

        $page_data['adder_other_course_details'] = $adder_other_course_details;
        ...
        if(!empty($course_details['video_url']))
        {
            ...
            try {
                ...
                // return a json object if upload fails
                //die("Error: " . $e->getMessage());
            }

          /*$result = $s3->getObject(array(
              'Bucket' => $bucketName,
              'Key'    => $course_details['video_url']
          ));*/

          $s3_video_url = $s3->getObjectUrl($bucketName, $course_details['video_url']);
      ...
      else
      {
        ...
        $page_data['s3_video_url'] = $s3_video_url;
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function lesson($slug = "", $course_id = "", $lesson_id = "") {
       ...
       if(!empty($lesson_id))
       {
           ...
           $lesson_thumbnail_url = $this->crud_model->get_lesson_thumbnail_url($lesson_id);
            ...
            if(!empty($lesson_details['video_url']) && $lesson_details['video_type']=="video_upload")
            {
              ...
              $s3_video_url = $s3->getObjectUrl($bucketName, $lesson_details['video_url']);
    ...
    public function search($search_string = "") {
        ...
        // +++++++  start of newly added for Showing x to y of z entries  +++++++++++

        if($page_data['courses'])
        {
           $total_rows=count($page_data['courses']);
        }
        ...
        $page_data['limit']  = $total_rows;
        $page_data['offset']  = $page;
        $page_data['total_records']  = $total_rows;

        // +++++++ end of newly added for Showing x to y of z entries  +++++++++++
    ...
    public function submit_quiz() {
        ...
        $page_data['quiz_details'] = $this->crud_model->get_quiz_details($quiz_id);
        $user_id = $this->session->userdata('user_id');
        $page_data['user_details'] = $this->user_model->get_all_user($user_id)->row_array();

        //echo "<pre>";
        //print_r($page_data['user_details']);exit();


        // +++++++++++++ Start of email send to user +++++++++++++++

        $quiz_email_subject ='vTrain - Quiz Result';
        $quiz_email_body = $this->load->view('lessons/quiz_result_email', $page_data, TRUE);
        $quiz_email_result = $this->load->view('lessons/quiz_result', $page_data, TRUE);
        require_once(APPPATH.'third_party/mpdf/mpdf.php');
        $mpdf=new mPDF();
        $mpdf->WriteHTML($quiz_email_result);
        $pdf_filename=time().".pdf";
        $mpdf->Output(FCPATH."/uploads/quiz_email_pdfs/".$pdf_filename, 'F');
        $attachment= FCPATH."/uploads/quiz_email_pdfs/".$pdf_filename;
        $this->email_model->send_mail_on_quiz_complete($quiz_email_subject, $quiz_email_body, $attachment);

        // +++++++++++++ End of email send to user +++++++++++++++++
